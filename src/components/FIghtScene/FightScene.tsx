...
const FightScene: React.FC = () => {
  ...
  const passTurn = () => {
    ...
    if (currentTurn < fightOrder.length - 1) dispatch(setTurn(newTurn + 1));
    else dispatch(setTurn(0));
  };

  const handleSkillClick = (index: number) => {
    if (allyArr[0].getskillsCooldown()[index] > 0) return;
    ...
    return (
      <div className="enemys">
        {enemyArr.map((item, i) => {
          return (
            <Enemys
              enemyImg={item.getImgBig()}
              enemyHp={enemiesHp[i]}
              maxEnemyHp={item.getMaxHp()}
              enemyIndex={i}
              key={i}
            />
          );
  ...
  const getSkills = React.useMemo(() => {
    return (
      <ul
        className="fight-scene__skills-panel"
        style={fightOrder[currentTurn].getIsNpc() ? { filter: 'grayscale(1)' } : {}}
      >
        <li className="skills__item" onClick={() => handleSkillClick(0)}>
          <img
            src={allyArr[0].getSkillImgs()[0]}
            alt="img"
            style={allyArr[0].getskillsCooldown()[0] !== 0 ? { filter: 'grayscale(1)' } : {}}
          />
        ...
        <li className="skills__item" onClick={() => handleSkillClick(1)}>
          <img
            src={allyArr[0].getSkillImgs()[1]}
            alt="img"
            style={allyArr[0].getskillsCooldown()[1] !== 0 ? { filter: 'grayscale(1)' } : {}}
          />
        ...
        <li className="skills__item" onClick={() => handleSkillClick(2)}>
          <img
            src={allyArr[0].getSkillImgs()[2]}
            alt="img"
            style={allyArr[0].getskillsCooldown()[2] !== 0 ? { filter: 'grayscale(1)' } : {}}
          />
        ...
        <li className="skills__item" onClick={() => handleSkillClick(3)}>
          <img
            src={allyArr[0].getSkillImgs()[3]}
            alt="img"
            style={allyArr[0].getskillsCooldown()[3] !== 0 ? { filter: 'grayscale(1)' } : {}}
          />
  ...
  const getItems = React.useMemo(() => {
    return isInventoryOpen ? (
      <Inventory
        closeInventory={handleOpenCloseInventory}
        setPlayerHp={setPlayerHp}
        classIfCharWindowOpen={isCharacterWindowOpen ? '_character-window-open' : ''}
      />
    ) : (
      ''
    );
  ...
  const getCharWin = React.useMemo(() => {
    return isCharacterWindowOpen ? (
      <CharacterWindow
        closeCharacterWindow={handleOpenCloseCharacterWindow}
        classIfInventoryOpen={isInventoryOpen ? '_inventory-open' : ''}
      />
    ) : (
      ''
    );
...
export default React.memo(FightScene);
